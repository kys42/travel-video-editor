...
## 2026-09-08 — 기본 화면을 촘촘한 행 목록으로 변경

- 큰 선택 패널을 기본 노출하지 않고 썸네일·시간·제목·요약·대사를 행에 표시한다.
- 선택한 행 아래에서만 상세 확장, 재클릭/닫기/Escape로 접기. 타임라인과 정확한 구간 이동 유지.
- 브라우저에서 10행 목록, 4.730–18.890초 확장·모니터 연결·접기 확인.

## 2026-09-08 — 근거를 바로 읽는 클립 작업대

- 촘촘한 행에 대상·대화·인물 관찰·검토 상태와 필터를 추가했다. 시간 비율 타임라인에 제목과 시작 시각을 표시하며, 필터에서도 시간 위치를 보존한다.
- 상세는 작은 프레임, 설명·대사, 관찰 근거를 폭에 따라 2–3열로 배치한다. 인물·원음·품질을 다시 펼치는 단계를 없애고 재생·맥락 재생·접기를 한 줄에 모았다.
- 기존 evidence로 목록 신호를 계산하므로 추가 LLM 호출은 없다. 신원·얼굴 부재를 확정하지 않는다. 독립 리뷰를 반영해 동일한 대사라도 화면 관찰과 더 구체적인 시간 근거는 보존한다.
- 사용 계약을 docs/video-library.md와 DESIGN.md에 반영했다. PR #21 미병합 상태로 검토용 worktree와 로컬 웹을 유지한다. Notion 보드 갱신 대기.
- 검증: 전체 135개 테스트·Python 린트·JS 구문 검사·UI 레이아웃 스캔 통과. 실제 브라우저에서 17→6개 필터, 원본 범위, Escape 접기, Review/Rough Cut의 즉시 근거 표시와 넓은/좁은 배치를 확인했다. 파일럿과 하루 비교 웹 HTML을 재생성했다.

## 2026-09-08 — 최종 리뷰와 새 세션용 추출 인계

- 사용자가 현재 결과를 확인한 뒤 PR #21 리뷰·머지를 승인했다. 전체 PR 독립 리뷰에서 발견한 빈 전역 beat 배열의 구형 후보 누락을 수정하고 회귀 검증을 추가했다.
- docs/reusable-scene-library-runbook.md에 날짜 선택, 현재 입력 manifest, 준비/LLM 리뷰/finisher/라이브러리 생성, 검수와 재개 조건을 연결했다. 설치 스킬의 재사용 장면 지침을 Git 저장본에 반영하고 양쪽을 동기화했다.
- 397개 대상의 현재 입력 파일 존재와 accepted grouping을 읽기 전용으로 검증했다. 실제 3그룹 packet 재생성에서 clip_evidence_required=true를 확인했다. 새 날짜 전체 분석이나 추가 LLM 리뷰를 실행한 것은 아니다.
- 전체 136개 테스트·Python 린트·스킬 검증 통과. 독립 리뷰에서 새 인계 명령과 runner 경로/상태 계약 일치 확인. 후속 결과는 기존 정본과 별도 run에 저장한다.
