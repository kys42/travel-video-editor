...
스킬을 처음 호출하면 **환경 점검 → 필요한 설치 → 영상 소스 확인 → 작업 범위와 편집 방향 선택**을 함께 진행합니다. Apple 음성 모델은 실제 설치 상태를 확인하고, 기존 분석 자료로 편집할 때는 불필요한 음성 모델 설치를 건너뜁니다.

> 처음 쓰는 환경이야. 필요한 준비를 점검하고 설치를 도와줘.
> 영상 소스를 확인한 다음, 장면 추출만 할지 하이라이트까지 만들지 같이 정하자.
