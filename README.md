...
- 2026-09-04: 검증된 편집 proposal의 후보별 검토·부분 적용과, 선택 source 구간만 최대 12프레임 시트·검토 자막·원시 STT로 읽는 심층 근거 도구 구현
...
검토 라이브러리를 로컬 API와 함께 열면 오른쪽 `AI Editor`에서 장면 검색, 선택 구간 심층 근거 조회, 편집 proposal 검토·부분 적용과 수정 요청을 할 수 있습니다. 에이전트는 원본 파일이나 셸에 편집 권한을 받지 않고, 문서화된 도구만 제안합니다. 서버가 proposal 후보와 도구 입력을 검증하고 모든 편집 변경을 새 SQLite revision으로 기록합니다. 이미지 판단이 더 필요할 때도 전체 영상을 보내지 않고 기존 분석 프레임 또는 프록시에서 만든 최대 12프레임 contact sheet 한 장만 사용합니다.
