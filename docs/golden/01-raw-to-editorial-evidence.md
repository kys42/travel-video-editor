...
## 편집용 추출 근거 확장 (선택 적용 v1)

`--clip-evidence`는 같은 통합 모델 패스에 사건 구조·핵심 구간·화면 관찰/발화 언급·장면 내부 인물과 화자·상호작용·원음 추정·샘플 품질 근거를 추가한다. Apple Vision의 얼굴/사람 양성 검출은 샘플 근거로 보존하며 신원이나 얼굴 부재 판정으로 사용하지 않는다. [정식 확장 계약과 실행 명령](../clip-editorial-evidence.md)을 따른다.


## 장면 라이브러리 우선 추출

추출 결과는 하이라이트 선별본이 아니다. 평범한 행동·조용한 볼거리·전환도
원본 시간과 근거를 붙여 보존하고, 목적별 선택과 합치기는 2단계에서 한다.
coarse group은 모델의 맥락 단위이며 기본 편집 클립 길이를 뜻하지 않는다.

- 행동, 관찰 대상, 대화 주제, 상호작용 단계, 반응이 바뀌는 근거에서 beat를 나눈다.
- 한 발화나 지속 행동을 임의의 5초 격자 또는 후보 개수 목표로 자르지 않는다.
- 장면 설명은 환경·대상·행동 순서·대화 변화·반응을 구체적으로 보존한다.
  제목을 반복하거나 한 줄 요약으로 대체하지 않는다.
- clip_evidence의 항목은 관찰/발화/추정을 구분한다. 빈 항목을 수량 채우기용
  추측으로 채우지 않는다. 모르는 것은 미평가로 드러낸다.
- JSON 참조 검증 통과와 추출 품질 통과를 분리한다. 대표 자산의 구간 경계와
  정보 회수율을 실제 근거와 대조한 뒤 전체 backfill 여부를 결정한다.

후보 export의 `library_audit`는 긴 단일 후보, 반복된 설명, 짧은 장면 설명을
검토 신호로 남긴다. 30초/60자 기준은 검토 라우팅용 휴리스틱일 뿐이며,
자동 분할·품질 점수·합격 판정에 사용하지 않는다. 긴 연속 장면은 정당할 수 있다.
