...
통합 리뷰의 기본 지침은 하이라이트 압축이 아닌 재사용 라이브러리 구축이다.
후보 export는 `library_audit`에 그룹별 후보 수, 설명 길이, 미평가 필드 수와
검토 신호를 함께 기록한다. 기존 JSON은 호환되며 이 검사는 추가 모델 호출이 없다.
서로 다른 언어의 글자 수를 품질 척도로 비교하거나 빈 배열을 누락 오류로 단정하지 않는다.

웹은 후보별 행동·반응, 대상, 상호작용, 원음, 품질, 익명 인물을 표시하며
주장 시간과 관찰/발화/추정 구분을 유지한다. 설명 텍스트도 장면 검색 대상이다.
표시되는 원음 추정은 BGM 믹싱 지시가 아니며, 인물 관찰은 얼굴 제외 승인이 아니다.

실험은 기존 결과와 별도 run에 저장한다. 선택한 자산에서 분할의 구체적 변화,
발화·시각 순간의 보존, 원본 시간과 참조 검증을 함께 보고한다. 후보 수 증가만으로
개선이라고 판정하거나 이전 하루 분석을 자동 대체하지 않는다.

파일럿에서 제목만 바꾼 반복 문구와 공통 근거 문구가 정보량을 부풀리는 실패를 확인했다.
`near_duplicate_beat_summary`와 `repeated_evidence_across_beats`도 검토 신호로 기록한다.
긴 연속 행동에서 반복 관찰이 정당할 수 있으므로 자동 오류 판정은 하지 않는다.
