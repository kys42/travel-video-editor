> 2026-09-08 이전 README: 초기 구현 이력과 상세 CLI 예시를 보존한 문서입니다. 현재 소개와 시작점은 [프로젝트 README](../../README.md)를 참고하세요.
...
- [편집용 정보 추출과 재사용 클립 후보 계약](../clip-editorial-evidence.md)
- [새 세션에서 날짜별 장면 라이브러리 만들기](../reusable-scene-library-runbook.md)

1. [Golden 01 — 원본 영상에서 편집 근거까지](../golden/01-raw-to-editorial-evidence.md)
2. [Golden 02 — 편집 근거에서 하이라이트까지](../golden/02-editorial-evidence-to-highlight.md)
...
- [도구·MCP·스킬 조사](../research-and-tooling.md)
- [권장 아키텍처](../architecture.md)
- [토큰 효율적인 전수 장면 타임라인](../token-efficient-scene-timeline.md)
- [Phase 1 구현과 파일럿 결과](../phase1-pipeline.md)
- [영상 단위 요약과 시간순 다중 영상 라이브러리](../video-library.md)
- [채팅 기반 AI 영상 편집 플랫폼 설계](../agentic-editor-platform.md)
- [Editor Service 실행 가능한 기술 계약](../technical/editor-service.md)
- [AI Editor 기능 및 운영 가이드](../technical/agent-capabilities.md)
- [Editor Agent 단일 계약](../contracts/editor-agent.v1.json)
- [SSE 이벤트 계약](../contracts/agent-events.v1.schema.json)
- [`kyungdoc/video-summary` 검토](../upstream-video-summary-review.md)
- [T7 원본 사전 점검](../source-assessment-2026-09-02.md)
- [소스 미디어와 작업 미디어 위치](../media-locations.md)
- [파일럿 실행 계획](../pilot-plan.md)
- [결정 기록과 미결 사항](../decisions.md)
...
현재 4K 원본은 T7, 1080p 작업 프록시는 ExternalSSD에 둡니다. 정확한 절대 경로, 현재 파일 수와 원본/프록시의 역할은 [미디어 위치 문서](../media-locations.md)를 기준으로 합니다. 이 저장소에는 코드·설정·작은 메타데이터만 보관합니다.
...
[Phase 1 문서](../phase1-pipeline.md)를 참고합니다.
...
Vision 운영 정책은
[멀티모달 편집 경계 제안 계약](../boundary-proposal-contract.md)을 참조합니다.
...
기본값은 프레임을 `index.html`에 내장해 파일 하나만 열어도 동작합니다. 영상은 넣지 않고 상단 공용 미리보기 덱만 남기며, 향후 저화질 프록시 하나를 이 영역에 연결합니다. 같은 로컬 파일 구조에서만 쓸 때는 `--assets relative`로 HTML 크기를 줄일 수 있습니다. [웹 타임라인 설계](../web-timeline.md)에 재사용 구조와 프록시 연결 방식을 기록했습니다.
...
상세 계약과 실제 음식 시퀀스 결과는 [다중 영상 라이브러리 문서](../video-library.md)에 있습니다.
