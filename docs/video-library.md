...
기본 화면은 썸네일·제목·시작 시간·길이·핵심 설명·대사 미리보기를 담은 촘촘한
행 목록이다. 큰 사진이나 상세 패널은 처음에는 표시하지 않는다. 행이나 길이 비율
타임라인을 누르면 선택한 행 바로 아래에 하나의 상세 패널을 펼친다.
같은 행, 상세 접기 버튼, Escape로 닫을 수 있다. 방향키/Home/End는 행 사이의
초점을 이동하고 Enter/Space로 펼친다.

선택은 자동 재생 없이 원본 범위를 모니터에 연결한다. 후보 재생과 맥락 재생
버튼은 펼친 상세에 유지한다. 좁은 화면에서도 행 목록을 유지하며 펼친 상세만
세로로 배치한다. 인물·원음·품질 근거 및 전체 장면 대화는 명시적 접기에 보존한다.
