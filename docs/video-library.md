...
장면 해석·보정 대화 전체·특이 포인트는 기계 분석 근거와 독립된
`전체 장면 설명·대화·편집 포인트`에서 확인한다. 클립별 설명·대화·관찰 정보는
선택한 행 바로 아래에 표시한다.


### 클립 탐색 작업대

기본 화면은 썸네일·제목·시작 시간·길이·핵심 설명·대사 미리보기·대상·검토 상태를
정렬한 촘촘한 행 목록이다. 대화, 인물 관찰, 검토 필요 필터로 후보를 좁힐 수 있다.
길이 비율 타임라인에는 제목과 시작 시간을 표시한다. 필터에서 제외된 구간도
위치와 길이를 유지하며 비활성화해 전체 시간 흐름이 바뀌지 않게 한다.

행이나 타임라인을 누르면 선택한 행 바로 아래에 하나의 상세 패널을 펼친다.
같은 행, 상세 접기 버튼, Escape로 닫을 수 있다. 방향키/Home/End는 행 사이의
초점을 이동하고 Enter/Space로 펼친다. 필터 사용 중에는 보이는 행 사이에서 이동한다.

선택은 자동 재생 없이 원본 범위를 모니터에 연결한다. 후보 재생과 맥락 재생
버튼과 접기를 한 줄에 둔다. 펼친 상세는 작은 프레임·설명과 대사·관찰 정보를
사용 가능한 폭에 따라 2–3열로 배치한다. 대상·행동, 상호작용, 원음, 화면 품질,
인물 관찰은 값이 있으면 추가 클릭 없이 보인다. 빈 항목을 반복해서 나열하지 않는다.

목록 신호는 기존 evidence에서 표시용으로 계산한다. 화면 관찰과 발화 언급·추정을
구분하며, 빈 인물 배열로 얼굴 부재나 우리 얼굴 제외를 판정하지 않는다.
클립 대사와 시간·근거가 모두 같은 발화 흐름만 중복 표시를 생략하고, 다른 관찰
근거나 세부 시간은 유지한다. 추출 JSON과 원본 구간은 이 표시 변경으로 바뀌지 않는다.
