...
후보가 있는 장면은 길이 비율 타임라인, 시간순 썸네일 목록, 선택한 후보 한 개의
상세 패널로 표시한다. 목록과 상세 영역은 각각 스크롤되어 후보 수에 따라 페이지가
계속 길어지지 않는다. 좁은 화면에서는 썸네일이 가로로 이동한다.

썸네일/타임라인 선택은 원본 구간을 모니터에 로드하며 자동 재생하지 않는다.
후보 재생과 앞뒤 맥락 재생은 명시적 버튼으로 유지한다. 방향키와 Home/End로
후보 탭을 이동할 수 있다. 미평가 정보의 반복 목록은 없애고, 관찰 근거·익명 인물은
접힌 상세에 유지한다. 샘플의 초점/흔들림/가림/노출 주의는 요약 표시한다.
전체 장면 설명과 대화는 별도 접기에서 보존하며, 데이터 추출 결과는 변경하지 않는다.
